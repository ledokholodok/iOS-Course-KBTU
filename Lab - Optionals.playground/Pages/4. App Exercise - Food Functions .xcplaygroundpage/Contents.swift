...
func getMeal(_ typeOfMeal: String) -> Meal? {
    if let meal = meals[typeOfMeal] {
        return meal
    } else {
        return nil
    }
 ...
 */
func checkMealLog(_ arrMeal: [String: Meal]) -> [String: Any]? {
    ...
    if !arrMeal.isEmpty {
        for item in arrMeal {
            ...
            UserDefaults.standard.set(
                newArrMeals[item.key],
                forKey: item.key
            )

            result[item.key] = UserDefaults.standard.value(
                forKey: item.key
            )
        }
    }
...
print(checkMealLog(meals) ?? "Nothing is here")
